...
| Cron `sync-tick` (cola → sync + Telegram) | ⚠️ Bearer = `CRON_SECRET` en Cron |
| Cron `enqueue-due` (sondas cada 5 min) | Migración `00011`; ver [POLLING.md](./POLLING.md) |
...
|------|----------------|-----|
| `enqueue-due` | `*/5 * * * *` | SQL: `select enqueue_due_sync_jobs();` |
